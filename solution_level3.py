...
def main() -> None:
    ...
    # ── Phase 2: Gather ore at N9 (11 × yield-6 = 66 ore) ────────────────────
    travel_to("N9")
    for _ in range(11):
        tick[0] += 3   # mine gather-time
        add("gather")

    # ── Phase 3: Buy construction materials ───────────────────────────────────
    # Material requirements (all 15 towns × 11 upgrades + 2 tools):
    #   planks     345  (upgrades only)
    #   thatch      60
    ...
    #   kiln-glass  60
    #   nets        30
    #   iron-fittings 32  (30 police-stations + 2 boots)
    #   -- input resources --
    #   wood  1022  (planks×694 + kiln-glass×120 + fencing×180 + iron-fittings×34)
    ...
    #   ore     64  (iron-fittings×34×2 + boots×4 [already gathered 78])
    travel_to("Demacia")
    buy("wood",  1022, "Piltover")
    ...
    # ── Phase 4: Build six towns early, then finish components at Targon ─────
    # Deferring every build until all components are crafted wastes thousands of
    # ticks of upgraded production and civic Enteloot.  The first batch covers
    # six complete towns plus boots; the remaining batch is crafted at Targon,
    # which also has crafting affinity.
    upgrades = ("farmhouse", "fertilised-fields", "rec-center",
                "fire-station", "school", "library", "police-station",
                "pier", "quarry", "woodlands", "pottery-house")
    build_times = {"farmhouse": 3, "fertilised-fields": 3, "rec-center": 4,
                   "fire-station": 4, "school": 5, "library": 5,
                   "police-station": 5, "pier": 3, "quarry": 3,
                   "woodlands": 3, "pottery-house": 3}
    early_towns = ["Demacia", "Noxus", "Piltover", "Ionia", "Freljord", "Targon"]
    late_towns = [town for town in TOWNS if town not in early_towns]

    for item, qty in [("planks", 138), ("thatch", 24), ("mortar", 174),
                      ("bricks", 174), ("stone-blocks", 60), ("kiln-glass", 24),
                      ("rope", 80), ("fencing", 36), ("nets", 12),
                      ("iron-fittings", 14)]:
        craft(item, qty)
    craft("boots", 1)
    boots_crafted[0] = True

    for town in early_towns:
        travel_to(town)
        for upgrade in upgrades:
            tick[0] += build_times[upgrade]
            add("build", upgrade=upgrade)

    # Targon is a crafting-affinity town, so no return trip to Demacia is needed.
    for item, qty in [("planks", 207), ("thatch", 36), ("mortar", 261),
                      ("bricks", 261), ("stone-blocks", 90), ("kiln-glass", 36),
                      ("rope", 117), ("fencing", 54), ("nets", 18),
                      ("iron-fittings", 18)]:
        craft(item, qty)

    for town in late_towns:
        travel_to(town)
        for upgrade in upgrades:
            tick[0] += build_times[upgrade]
            add("build", upgrade=upgrade)

    # The pickaxe is intentionally omitted: all ore was gathered before it could
    # be crafted, so it would consume ticks and components without any benefit.
